...
import importMaterialTransaction from "./IMPORT_WAREHOUSE_OUT.xlsx";
import HospitalService from "../../../../../services/HospitalService";

const schema = yup.object(SCHEMA_ADD_BLOOD).required();

const AddWareHouseOut = ({ param }) => {
  // ** States
  ...
  const onSubmit = async (values) => {
    if (isObjEmpty(errors)) {
      ...
      const result = await BloodService.addBlood({
        transactionCodes: "OUT" + values?.transactionCodes?.trim(),
        bloodtype: 2,
        ...
        if (values?.items?.length > 0) {
          const promiseItem = values?.items?.map((item) => {
            return BloodDetailService.addBloodDetail({
              transactionCode: "OUT" + values?.transactionCodes?.trim(),
    ...
    <Card>
      <CardBody>
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          <CardTitle className="mt-2">Chi tiết xuất kho</CardTitle>
          <Row className="mb-2">
            <Col sm="10">
...
export default AddWareHouseOut;
