...
const RegisterDonateBloodList = () => {
  ...
  const handleGetlist = async (value) => {
    setLoading(true);
    const result = await RegisterDonateBloodService.getListRegisterDonateBlood(
      params
    );
              ...
              <Row>
                ...
                <Col md="4">
                  ...
                  <Row className="mt-2">
                    ...
                    {/* {isAuthCreate && (
                      <Link to="/apps/register_donate_blood/add">
                        <Button color="dark" outline tag={Label} size="md">
                          <span className="align-middle ms-25">Tạo mới</span>
                        </Button>
                      </Link>
                    )} */}
                  </Row>
