...
import { exportHeaderMyJob } from "../../../../../constants/export";
...
const TaskList = () => {
  ...
  useEffect(() => {
    ...
    setCurrentPage(0);
  }, [selectedStatus, selectedWorkStatus, selectedJobfield, selectedJob]);
          ...
          <div  style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            {isAuthDelete &&
              ...
              (userData?.role === "ADMIN" ||
                userData?.code === row?.user_code) && (
                <Badge
                style={{ width: "80px", display: "inline-block", textAlign: "center", marginTop:"5px", cursor: "pointer" }}
                  onClick={() => handleDeleteMyjob(row?.id)}
                  color="danger"
                >
                  Xoá
                </Badge>
  ...
  const handleExportXLSX = () => {
    ...
    let tableXLSX = [
      {
        A: exportHeaderMyJob.NAME,
        B: exportHeaderMyJob.JOB,
        C: exportHeaderMyJob.USER,
        D: exportHeaderMyJob.SEQUENCE,
        E: exportHeaderMyJob.PROCESSDATE,
        F: exportHeaderMyJob.DEALINE,
        G: exportHeaderMyJob.DESCRIPTION,
        H: exportHeaderMyJob.WORKSTATUS,
      },
    ];
    ...
    //title cho bảng
    tableXLSX = [{ A: exportHeaderMyJob.EXPORT_TITLE }].concat(tableXLSX);
    var wb = XLSX.utils.book_new();
    var ws = XLSX.utils.json_to_sheet(tableXLSX, {
      skipHeader: true,
    });

    XLSX.utils.book_append_sheet(wb, ws, exportHeaderMyJob.EXPORT_WS);

    XLSX.writeFile(wb, exportHeaderMyJob.EXPORT_WB);
  };
      ...
      <Fragment>
        <Card>
          ...
          <DataTable
            ...
            columns={columns}
            className="react-dataTable table-my-job"
            paginationPerPage={5}
